...
def train_model(device, num_epochs, trainloader, image_size = 28, channel_size = 1, patch_size = 7, embed_size = 512, num_heads = 8, classes = 10, num_layers = 3, hidden_size = 256, dropout = 0.2, save=True): 
...
def deactivate_dropout_layers(model):
  ...
  model.dropout_layer.p = 0.
  for i in range(3):
    model.encoders[i].attention.dropout_layer.p = 0.
    model.encoders[i].mlp[2].p = 0.
    model.encoders[i].mlp[4].p = 0.


def simec_vit(model, starting_img, n_iterations, delta = 5e-2, threshold = 1e-2, print_every_n_iter=100):
  ...
  for i in range(n_iterations): 
    ...
    clamp_lower  = simec_input < -1.
    simec_input[clamp_lower] = -1.
    clamp_upper  = simec_input > 1.
    simec_input[clamp_upper] = 1.
