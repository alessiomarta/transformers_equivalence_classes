...
class VisionEncoder(nn.Module):
    ...
    def __init__(
        self, embed_size: int, num_heads: int, hidden_size: int, dropout: float = 0.1
    ):
        ...
        self.attention = MultiHeadAttention(
            self.embed_size, self.num_heads, dropout=dropout
        )
    ...
    def __init__(
        ...
        for layer in range(self.num_layers):
            self.encoders.append(
                VisionEncoder(
                    self.embed_size, self.num_heads, self.hidden_size, self.dropout
                )
            )
